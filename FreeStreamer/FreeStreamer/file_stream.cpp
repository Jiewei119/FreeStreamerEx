...
bool File_Stream::open(const Input_Stream_Position& position)
{
    ...
    if (m_data) {
        const UInt8 *bytes = CFDataGetBytePtr(m_data);
        CFIndex length = CFDataGetLength(m_data);
        if (!(m_readStream = CFReadStreamCreateWithBytesNoCopy(kCFAllocatorDefault,bytes,length, kCFAllocatorDefault))) {
            goto out;
        }
    }
    else {
...
void File_Stream::setData(CFDataRef data)
{
    if (m_data) {
        CFRelease(m_data);
    }
    if (data) {
        m_data = (CFDataRef)CFRetain(data);
    } else {
        m_data = NULL;
    }
}
...
bool File_Stream::canHandleData(CFDataRef data)
{
    if (!data) {
        return false;
    }
