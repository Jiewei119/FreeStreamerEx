...
- (void)setData:(NSData *)data
{
    ...
    @synchronized (self) {
        ...
        _audioStream->setData((__bridge CFDataRef)_data);
    }
...
{
    NSAssert([NSThread isMainThread], @"FSAudioStream.initWithData needs to be called in the main thread");
...
- (void)setData:(NSData *)data
{
    NSAssert([NSThread isMainThread], @"FSAudioStream.setData needs to be called in the main thread");
...
- (NSData*)data
{
    NSAssert([NSThread isMainThread], @"FSAudioStream.data needs to be called in the main thread");
...
- (void)playFromData:(NSData*)data
{
    NSAssert([NSThread isMainThread], @"FSAudioStream.playFromData needs to be called in the main thread");
    
    [_private playFromData:data];
}
