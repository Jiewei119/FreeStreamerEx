...
- (void)dealloc
{
    if (self.audioController.enableDebugOutput) {
        if (self.data) {
            NSLog(@"[FSAudioController.m:%i] FSAudioStreamProxy.dealloc", __LINE__);
        }
        else {
...
- (void)dealloc
{
    ...
    for (FSAudioStreamProxy *proxy in _streams) {
        if (self.enableDebugOutput) {
            if (proxy.data) {
                NSLog(@"[FSAudioController.m:%i] dealloc. Deactivating stream", __LINE__);
            }
            else {
...
- (void)audioStreamStateDidChange:(NSNotification *)notification
{
    ...
    if (state == kFSAudioStreamEndOfFile) {
        if (self.enableDebugOutput) {
            if (self.audioStream.data) {
                NSLog(@"[FSAudioController.m:%i] EOF reached", __LINE__);
            }
            else {
        ...
        if ([self hasNextItem]) {
            ...
            if (self.enableDebugOutput) {
                if (nextStream.data) {
                    NSLog(@"[FSAudioController.m:%i] Preloading", __LINE__);
                }
                else {
            ...
            if ([self.delegate respondsToSelector:@selector(audioController:allowPreloadingForStream:)]) {
                if ([self.delegate audioController:self allowPreloadingForStream:nextStream]) {
                    ...
                    if (self.enableDebugOutput) {
                        if (nextStream.data) {
                            NSLog(@"[FSAudioController.m:%i] Preloading disallowed for stream", __LINE__);
                        }
                        else {
        ...
        if (self.enableDebugOutput) {
            if (self.audioStream.data) {
                NSLog(@"Stream stopped. No next playlist items. Deactivating audio session");
            }
            else {
        ...
        if (self.enableDebugOutput) {
            if (self.audioStream.data) {
                NSLog(@"Stream failed. Deactivating audio session");
            }
            else {
...
- (void)deactivateInactivateStreams:(NSUInteger)currentActiveStream
{
    ...
    for (FSAudioStreamProxy *proxy in _streams) {
        if (streamIndex != currentActiveStream) {
            if (self.enableDebugOutput) {
                if (proxy.data) {
                    NSLog(@"[FSAudioController.m:%i] Deactivating stream", __LINE__);
                }
                else {
...
- (void)play
{
    ...
    if ([self.playlistItems count] > 0) {
        if (self.currentPlaylistItem.data) {
            self.audioStream.data = self.currentPlaylistItem.data;
        }
        else {
            ...
            // 音频文件解密处理
            [self audioDataDecryptProcess];
        }
    } else {
        if (self.currentPlaylistItem.data || self.data) {
            if (self.currentPlaylistItem.data) {
                self.audioStream.data = self.currentPlaylistItem.data;
            }
            else {
                self.audioStream.data = self.data;
            }
        }
        else {
...
// 音频文件解密处理
- (void)audioDataDecryptProcess {
    if (self.audioStream.url && self.audioDataDecrypt) {
        NSData* audioData = self.audioDataDecrypt(self.audioStream.url);
        self.currentPlaylistItem.data = audioData;
        self.audioStream.data = audioData;
    }
}
...
- (void)playFromPlaylist:(NSArray *)playlist itemIndex:(NSUInteger)index
{
    ...
    for (FSPlaylistItem *item in playlist) {
        ...
        if (self.enableDebugOutput) {
            if (proxy.data) {
                NSLog(@"[FSAudioController.m:%i] playFromPlaylist. Adding stream proxy", __LINE__);
            }
            else {
...
- (void)addItem:(FSPlaylistItem *)item
{
    ...
    if (self.enableDebugOutput) {
        if (proxy.data) {
            NSLog(@"[FSAudioController.m:%i] addItem. Adding stream proxy", __LINE__);
        }
        else {
...
- (void)removeItemAtIndex:(NSUInteger)index
{
    ...
    if (self.enableDebugOutput) {
        ...
        if (proxy.data) {
            NSLog(@"[FSAudioController.m:%i] removeItemAtIndex. Removing stream proxy", __LINE__);
        }
        else {
...
-(void)playNextItem
{
    if ([self hasNextItem]) {
        if (self.enableDebugOutput) {
            if (self.audioStream.data) {
                NSLog(@"[FSAudioController.m:%i] playNexItem. Stopping stream", __LINE__);
            }
            else {
...
-(void)playPreviousItem
{
    if ([self hasPreviousItem]) {
        if (self.enableDebugOutput) {
            if (self.audioStream.data) {
                NSLog(@"[FSAudioController.m:%i] playPreviousItem. Stopping stream", __LINE__);
            }
            else {
